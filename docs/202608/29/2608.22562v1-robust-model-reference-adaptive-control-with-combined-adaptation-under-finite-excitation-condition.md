...
## 速览
**TLDR**：针对自适应控制中持续激励（PE）条件难以在线验证的问题，本文提出一种基于有限激励（FE）条件的鲁棒模型参考自适应控制方法。通过改进的Gram-Schmidt正交化过程构建记忆项，消除了收敛速率对激励水平的依赖，实现了多输入多输出非线性系统参数估计误差的指数级收敛，并给出了明确的误差最终界限。 \
**Motivation**：传统的持续激励条件难以在线保证，且现有基于有限激励的方法在收敛速率和鲁棒性方面仍存在局限。 \
**Method**：利用改进的Gram-Schmidt正交化构建记忆项，并结合复合自适应律来估计控制器增益。 \
**Result**：闭环跟踪和估计误差以指数级速率衰减至原点邻域，且衰减速率仅取决于用户定义的增益，与激励强度无关。 \
**Conclusion**：该方法有效解决了参数收敛对激励水平的依赖问题，提高了非线性系统自适应控制的鲁棒性和收敛性能。
