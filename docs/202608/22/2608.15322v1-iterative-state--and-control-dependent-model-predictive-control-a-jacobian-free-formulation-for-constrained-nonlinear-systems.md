...
tags: ["query:pmsm"]
score: 6.0
evidence: 约束非线性系统的模型预测控制
tldr: 本文提出一种针对受限非线性系统的迭代模型预测控制（MPC）算法，无需计算系统导数。通过将非线性动力学分解为状态和控制依赖系数（SCDC）的伪线性形式，将非凸优化转化为一系列受限线性二次规划问题。该方法在原点附近具有收敛性保证，计算复杂度随预测时域线性增长（O(l)），并能确保递归可行性和渐近稳定性，适用于实时性要求高的复杂控制任务。
selection_source: fresh_fetch
motivation: 旨在解决非线性MPC中雅可比矩阵计算复杂以及计算成本随预测时域呈立方增长的瓶颈问题。
method: 采用状态和控制依赖系数（SCDC）将非线性动力学重构为伪线性形式，并通过迭代求解受限线性二次规划子问题。
result: 理论证明了算法在原点附近的收敛性及最优性，且数值实验表明其计算效率显著优于传统的SQP方法。
conclusion: 该算法提供了一种无需导数、计算高效且具有严谨稳定性保证的非线性系统控制新方案。
---
