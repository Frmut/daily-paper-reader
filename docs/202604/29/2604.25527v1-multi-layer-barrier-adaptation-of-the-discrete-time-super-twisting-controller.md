...
## 摘要
在数字滑模控制实现中，离散化引起的抖振和采样间盲区会严重降低闭环性能，特别是在存在快速扰动的情况下。本文针对超螺旋滑模控制器的离散时间实现解决了这些挑战。基于障碍函数调制的超螺旋算法的最新研究成果，本文采用基于特征值的精确匹配方法，对一种采用多重障碍的嵌套架构进行了离散化。所得的离散时间控制器保留了在连续时间下建立的自适应和鲁棒特性，同时确保了采样层级上的一致稳定性行为。所提出的框架通过数值仿真得到了验证。结果突出了多层障碍自适应在离散时间滑模控制应用中的有效性。
