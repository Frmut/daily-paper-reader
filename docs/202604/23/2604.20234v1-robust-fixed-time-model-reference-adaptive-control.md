...
tags: ["query:pmsm"]
score: 6.0
evidence: 鲁棒固定时间模型参考自适应控制
tldr: 本文针对未知线性时不变系统，提出一种鲁棒固定时间模型参考自适应控制（MRAC）策略。该方法通过引入新型参数更新律，在不依赖严格持续激励（PE）条件的情况下，利用更易实现的初始/区间激励（IE）条件，实现了参数估计和跟踪误差的固定时间收敛。研究不仅简化了自适应控制的工程应用要求，还显著提升了系统应对参数不确定性和外部扰动的鲁棒性，并通过仿真验证了其优越性。
selection_source: fresh_fetch
motivation: 旨在解决传统自适应控制中收敛速度受限且高度依赖难以满足的持续激励（PE）条件的问题。
method: 在间接MRAC框架下设计了一种新型参数更新律，利用放宽的初始/区间激励（IE）条件来驱动参数估计。
result: 成功实现了参数估计和跟踪误差在固定时间内的收敛，且在存在参数不确定性和外部扰动时表现出良好的鲁棒性。
conclusion: 该方法降低了自适应控制的实际应用门槛，为线性系统提供了一种更高效、更实用的固定时间控制方案。
---

## 摘要
本文提出了一种模型参考自适应控制（MRAC）策略，旨在实现未知线性时不变系统参数估计和跟踪误差的固定时间收敛，且不依赖于持续激励（PE）条件。该策略对回归矩阵采用了限制较少的初始/区间激励条件，从而增强了在实际场景中的实用性和易实现性。我们的主要贡献是在间接 MRAC 框架下提出了一种新型参数更新律，确保一旦满足初始/区间激励条件，参数估计即可在固定时间内收敛。该方法在简化自适应控制实际要求的同时，保证了针对参数不确定性和外部扰动的鲁棒性能。仿真结果通过与现有文献的对比，验证了该方法的有效性。
