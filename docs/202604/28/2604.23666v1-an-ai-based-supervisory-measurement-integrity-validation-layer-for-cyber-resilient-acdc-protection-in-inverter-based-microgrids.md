...
title_zh: 一种用于基于逆变器的微电网中网络韧性交流/直流保护的基于人工智能的监控式测量完整性验证层
...
motivation: 逆变器型微电网高度依赖数字化通信，使得电流差动保护容易受到操纵测量流的虚假数据注入攻击，导致保护误动作。
method: 提出一种基于循环神经网络（RNN）的监控层，通过分析短窗口内的同步瞬时电流测量值，利用波形的时间结构特征来验证数据的物理一致性。
result: 在孤岛微电网环境下的多种故障和攻击场景中表现出高检测精度，并通过硬件在环（HIL）仿真验证了其满足保护动作的时间约束。
conclusion: 该方案为现代AC/DC微电网提供了一种无需改变网络拓扑或增加硬件成本的增强型网络韧性保护机制。
---

## 摘要
线路电流差动继电器 (LCDR) 是测量驱动型继电器，依靠时间同步的多相电流波形来推断交流和直流电力网络中的内部故障。然而，在基于逆变器的微电网中，对数字通信测量的日益依赖使 LCDR 容易受到虚假数据注入攻击 (FDIA) 的威胁。在这些攻击中，攻击者操纵远程测量流，以产生触发保护但物理上不一致的电流轨迹。本文通过引入一种测量完整性验证方案来解决这一新兴的测量完整性问题，该方案作为现代 LCDR 的监控仪表层运行。所提出的方案解释了继电器运行期间记录的同步瞬时电流测量的短窗口，并评估其物理一致性，以区分真实的故障诱导轨迹和网络操纵的测量流。使用仅继电器可用的电流测量值离线训练循环神经网络，并利用差动电流波形的时间结构；在电流幅度不再是可靠观测量的逆变器主导系统中，这种时间结构仍然具有信息性。该方法不需要额外的传感器、辅助保护元件或网络拓扑的先验知识，并且适用于交流和直流 LCDR，无需进行结构修改。所提出的测量验证方案在孤岛式基于逆变器的微电网中，在全面的故障和 FDIA 场景下进行了评估，证明了高检测精度的同时保持了继电器的可靠性。使用 OPAL-RT 实时仿真器进行的硬件在环验证确认，该方案满足保护时限约束，并能在现实运行条件下实时运行。
