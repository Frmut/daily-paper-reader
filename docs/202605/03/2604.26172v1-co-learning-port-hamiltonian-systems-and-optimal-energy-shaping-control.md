...
## 摘要
我们开发了一个基于物理信息的学习框架，用于从轨迹数据中实现端口-哈密顿（pH）系统的能量整形控制。所提出的方法通过交替优化和策略感知的数据采集，协同学习 pH 系统模型和最优能量平衡无源性控制（EB-PBC）。在每次迭代中，利用当前控制策略下采集的轨迹数据对系统模型进行细化，并在更新后的模型上重新优化控制器。这两个组件均由嵌入了 pH 动力学和 EB-PBC 结构的神经网络进行参数化，确保了能量交互方面的可解释性。学习到的控制器使闭环系统具有固有的无源性和可证明的稳定性，并在不抵消自然势能的情况下利用无源受控对象的动力学。耗散正则化在训练过程中强制执行严格的能量衰减，从而增强了对仿真到现实差距（sim-to-real gaps）的鲁棒性。所提出的框架在平面摆和扭转摆系统的状态调节及起摆任务上得到了验证。
