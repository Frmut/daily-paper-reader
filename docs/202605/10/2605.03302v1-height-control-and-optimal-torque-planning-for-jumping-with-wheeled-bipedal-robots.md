...
## 摘要
本文主要研究基于力矩规划和能耗优化的轮足双足机器人精确跳跃高度控制。由于跳跃过程中存在欠驱动、非线性估计和瞬时冲击等特性，轮足双足机器人跳跃高度的精确控制十分复杂。在现实中，机器人通常为了确保安全而以过高的高度跳跃，导致额外的电机损耗、更大的地面反作用力和更多的能量消耗。为解决这一问题，提出了一种新型轮足双足跳跃动力学模型（W-JBD）以实现精确的高度控制。该模型表现良好，但由于力矩存在显著的阶跃，不适用于实际机器人。因此，提出了用于力矩规划的贝叶斯优化方法（BOTP），该方法无需精确的动力学模型，且能在较少的迭代次数内获得最优力矩规划。BOTP 方法通过连续的力矩曲线，可减少 82.3% 的高度误差和 26.9% 的能量消耗。该结果在 Webots 仿真平台中得到了验证。基于 W-JBD 模型获得的力矩曲线来缩小搜索空间，BOTP 可以快速收敛（平均 40 次）。通过 W-JBD 模型与 BOTP 方法的协作，可以在合理的实验次数内实现实际机器人的高度控制。
