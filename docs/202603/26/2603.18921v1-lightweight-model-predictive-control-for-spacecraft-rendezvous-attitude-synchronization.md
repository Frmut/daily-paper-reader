...
## 摘要
本研究介绍了两种用于航天器交会同步过程中利用反作用轮进行姿态跟踪的轻量级模型预测控制（MPC）方法。这两种方法均基于一种新颖的姿态偏差公式，该公式允许对角速度使用固有的线性约束。我们开发了单环和双环 MPC；后者在内环中嵌入了一个稳定反馈控制器，从而产生一个线性时不变系统。两种控制器均使用 CasADi 实现（包括自动代码生成），在多种求解器上进行了评估，并在 Basilisk 天体动力学仿真框架中进行了验证。实验结果表明，在提高跟踪精度的同时，计算量和内存消耗也有所降低。最后，在 ARM Cortex-M7（代表新航天平台中使用的商用现货设备）上的嵌入式部署确认了这些方法的实时可行性，并突显了它们在资源受限的航天器交会任务中进行星载姿态控制的适用性。
